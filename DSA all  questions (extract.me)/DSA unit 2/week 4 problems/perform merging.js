...
function margeTwoAraay(N,arr1,arr2)
{
    // var arr= (arr1.join(" ")+" "+arr2.join(" ")).split(" ").map(Number).sort(function(a,b)
    // {
    //     return a-b
    // }).join(" ")
    // console.log(arr)
    
    //its a google ques right approach is below one becz array is already sorted
    ...
    if(bag[bag.length-1] != arr1[arr1.length-1]) // this not works we have to use while loops
    {
...
//       {
//         if(arr[j]<arr[i])             //for descending condition is arr[j]>arr[i]
//         {
//           swap(arr,i,j)
//         }
//       }
